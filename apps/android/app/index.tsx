  ...
  if (!user) {
    return (
      <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20, gap: 14 }}>
        ...
        <Text style={{ color: '#475569' }}>Sign in to sync your boxes across web and Android.</Text>
        <TextInput
          autoCapitalize="none"
        ...
        <Pressable style={buttonStyle} onPress={() => void submitAuth('sign-in')}>
          <Text style={buttonTextStyle}>Sign In</Text>
        </Pressable>
        <Pressable style={secondaryButtonStyle} onPress={() => void submitAuth('sign-up')}>
          <Text style={{ color: '#0f172a', fontWeight: '600' }}>Create Account</Text>
        </Pressable>
      </ScrollView>
    );
  }
    ...
    <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20, gap: 14 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
        <View>
          ...
          <Text selectable style={{ color: '#64748b' }}>{user.email}</Text>
        </View>
      ...
      {boxes.length === 0 && !isLoadingBoxes ? (
        ...
        <Link key={box.id} href={`/box/${box.id}`} asChild>
          <Pressable style={cardStyle}>
            <Text selectable style={{ fontSize: 18, fontWeight: '700' }}>Box #{box.id.slice(0, 8)}</Text>
            <Text selectable style={{ color: '#475569' }}>{box.assignedRoom || 'No room assigned'}</Text>
            <Text style={{ color: '#64748b' }}>{box.aiGeneratedTags?.length || 0} tags</Text>
          </Pressable>
        </Link>
      ))}
    </ScrollView>
