...
export default function BoxDetailScreen() {
  ...
  const loadBox = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : 'Could not load box.');
    }
  }, [id, user]);
  ...
  if (!box) {
    return <Centered><Text selectable>Box not found.</Text></Centered>;
  }
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20, gap: 16 }}>
      <Text selectable style={{ fontSize: 28, fontWeight: '700' }}>Box #{box.id.slice(0, 8)}</Text>
      {box.photoUrl ? <Image source={{ uri: box.photoUrl }} style={{ width: '100%', height: 240, borderRadius: 8 }} /> : null}
      <View style={{ alignItems: 'center', backgroundColor: 'white', borderRadius: 8, padding: 16 }}>
        <QRCode value={url} size={180} />
        <Text selectable style={{ marginTop: 10, color: '#475569' }}>{url}</Text>
      </View>
      <Info label="Room" value={box.assignedRoom || 'Not assigned'} />
      <Info label="Notes" value={box.manualDescription || 'No notes'} />
      <Info label="Tags" value={box.aiGeneratedTags?.length ? box.aiGeneratedTags.join(', ') : 'No AI tags'} />
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <View style={{ gap: 4 }}>
      ...
      <Text selectable style={{ color: '#0f172a', fontSize: 17 }}>{value}</Text>
    </View>
  );
}
