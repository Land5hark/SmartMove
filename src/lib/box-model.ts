...
export function sanitizeBoxRecord(id: string, value: unknown): Box {
  const record = value && typeof value === 'object' ? value as Partial<Box> : {};
  ...
  return {
    ...
    manualDescription: typeof record.manualDescription === 'string' ? record.manualDescription : undefined,
    aiGeneratedTags: stringArray(record.aiGeneratedTags),
    suggestedRoom: typeof record.suggestedRoom === 'string' ? record.suggestedRoom : undefined,
    assignedRoom: typeof record.assignedRoom === 'string' ? record.assignedRoom : undefined,
