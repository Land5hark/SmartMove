...
async function uploadPhoto(userId: string, boxId: string, photoDataUrl?: string) {
...
export async function listBoxes(userId: string): Promise<Box[]> {
  ...
  return snapshot.docs.map(boxDoc => sanitizeBoxRecord(boxDoc.id, boxDoc.data()));
}

export async function getBox(userId: string, boxId: string): Promise<Box | null> {
...
export async function saveBox(userId: string, input: Partial<Box> & Pick<Box, 'id'>): Promise<Box> {
...
export async function importLegacyBox(userId: string, legacyBox: Box): Promise<Box> {
  const id = legacyBox.id || doc(boxesCollection()).id;
  return saveBox(userId, { ...legacyBox, id });
}

export async function createBox(userId: string, input: Omit<Partial<Box>, 'id'>): Promise<Box> {
